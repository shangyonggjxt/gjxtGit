...
function showButton(){
  ...
  for(i=0;i<a.length;i++) {
	...
	<body onLoad="showButton()">
	  ...
	  <div class="easyui-panel" title="组织机构||一级节点" style="width:800px;padding:0px 0px">
...
<table width="50%" cellspacing="1" bgcolor="#CCCCCC">
  <tr height="30px">
    <td align="center" bgcolor="#FFFFFF"><input type="button" name="Submit6" value="一级节点" onClick="query1()" class="easyui-linkbutton" style="font-size:14px;"></td>
    <td align="center" bgcolor="#FFFFFF"><input type="button" name="Submit62" value="二级节点" onClick="query2()" class="easyui-linkbutton" style="font-size:14px;"></td>
    <td align="center" bgcolor="#FFFFFF"><input type="button" name="Submit63" value="三级节点" onClick="query3()" class="easyui-linkbutton" style="font-size:14px;"></td>
    <td align="center" bgcolor="#FFFFFF"><input type="button" name="Submit64" value="四级节点" onClick="query4()" class="easyui-linkbutton" style="font-size:14px;"></td>
  </tr>
</table>
			...
			<table width="100%" cellspacing="1" bgcolor="#CCCCCC">
				<tr>
					...
					<td width="20%" bgcolor="#FFFFFF">
					  <input class="easyui-textbox" style="width:100%;height:30px;padding:12px" data-options="prompt:'请输入组织机构名称'"  name="groupFirst.group_name" id="groupFirst.group_name" value="${groupFirst.group_name}">
				  </td>
					<td width="20%" bgcolor="#FFFFFF">
											每页显示
				...
				<tr>
				  <td colspan="4" align="center" bgcolor="#FFFFFF"><s:property value="group_name" />
			    </td>
...
function update(group_id){
  ...
  var time=myDate.getMilliseconds();  
    ...
    if(!!window.ActiveXObject || "ActiveXObject" in window){
    window.showModalDialog(encodeURI("<%=request.getContextPath()%>/stationGroup.action?functionName=groupFirst_update_view&groupFirst.group_id="+group_id+"&time="+time),"","dialogWidth=700px;dialogHeight=500px;toolbar=no;menubar=no;scrollbars=no;resizable=no;location=no;status=no");
  query();
  }else{
  window.open(encodeURI("<%=request.getContextPath()%>/stationGroup.action?functionName=groupFirst_update_view&groupFirst.group_id="+group_id+"&time="+time),"","height=500,width=700,top=100,left=100,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no, status=no");
  query();
  }
...
function preSave(){
  ...
  var time=myDate.getMilliseconds();  
   ...
   window.showModalDialog(encodeURI("<%=request.getContextPath()%>/stationGroup.action?functionName=preSaveFirst&time="+time),"","dialogWidth=700px;dialogHeight=500px;toolbar=no;menubar=no;scrollbars=no;resizable=no;location=no;status=no");
   query();
  }else{
  
   window.open(encodeURI("<%=request.getContextPath()%>/stationGroup.action?functionName=preSaveFirst&time="+time),"","height=500,width=700,top=100,left=100,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no, status=no");
   query();
  }
}
  ...
  $.messager.confirm('提示信息', '确认要删除吗？', function(r){
		if (r){
           ...
           var time=myDate.getMilliseconds();  
             ...
             if(!!window.ActiveXObject || "ActiveXObject" in window){
             window.showModalDialog(encodeURI("<%=request.getContextPath()%>/stationGroup.action?functionName=delGroupFirst&groupFirst.group_id="+group_id+"&time="+time),"","dialogWidth=700px;dialogHeight=500px;toolbar=no;menubar=no;scrollbars=no;resizable=no;location=no;status=no");
           query();
  }else{
             window.open(encodeURI("<%=request.getContextPath()%>/stationGroup.action?functionName=delGroupFirst&groupFirst.group_id="+group_id+"&time="+time),"","height=500,width=700,top=100,left=100,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no, status=no");
           query();
