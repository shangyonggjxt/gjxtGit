...
function showButton(){
  ...
  for(i=0;i<a.length;i++) {
	...
	<div class="easyui-panel" title="组织机构||二级级节点" style="width:800px;padding:0px 0px">
			...
			<table width="100%" cellspacing="1" bgcolor="#CCCCCC">
				<tr>
					...
					<td width="20%" bgcolor="#FFFFFF">
					  <input class="easyui-textbox" style="width:100%;height:30px;padding:12px" data-options="prompt:'请输入组织机构名称'"  name="groupSecond.group_name" id="groupSecond.group_name" value="${groupSecond.group_name}">
				    </td>
					<td width="20%" bgcolor="#FFFFFF">
											每页显示
...
function update(group_id){
  ...
  var time=myDate.getMilliseconds();  
    ...
    if(!!window.ActiveXObject || "ActiveXObject" in window){
    window.showModalDialog(encodeURI("<%=request.getContextPath()%>/stationGroup.action?functionName=groupSecond_update_view&groupSecond.group_id="+group_id+"&time="+time),"","dialogWidth=700px;dialogHeight=500px;toolbar=no;menubar=no;scrollbars=no;resizable=no;location=no;status=no");
  query();
  }else{
  window.open(encodeURI("<%=request.getContextPath()%>/stationGroup.action?functionName=groupSecond_update_view&groupSecond.group_id="+group_id+"&time="+time),"","height=500,width=700,top=100,left=100,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no, status=no");
  query();
  }
...
function preSave(){
  ...
  var time=myDate.getMilliseconds();  
    ...
    if(!!window.ActiveXObject || "ActiveXObject" in window){
     window.showModalDialog(encodeURI("<%=request.getContextPath()%>/stationGroup.action?functionName=preSaveSecond&time="+time),"","dialogWidth=700px;dialogHeight=500px;toolbar=no;menubar=no;scrollbars=no;resizable=no;location=no;status=no");
   query();
  }else{
   window.open(encodeURI("<%=request.getContextPath()%>/stationGroup.action?functionName=preSaveSecond&time="+time),"","height=500,width=700,top=100,left=100,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no, status=no");
   query();
  }
    ...
    $.messager.confirm('提示信息', '确认要删除吗？', function(r){
		if (r){
            ...
            var time=myDate.getMilliseconds(); 
              ...
              if(!!window.ActiveXObject || "ActiveXObject" in window){
                          window.showModalDialog(encodeURI("<%=request.getContextPath()%>/stationGroup.action?functionName=delGroupSecond&groupSecond.group_id="+group_id+"&time="+time),"","dialogWidth=700px;dialogHeight=500px;toolbar=no;menubar=no;scrollbars=no;resizable=no;location=no;status=no");
            query();
  
               }else{
              window.open(encodeURI("<%=request.getContextPath()%>/stationGroup.action?functionName=delGroupSecond&groupSecond.group_id="+group_id+"&time="+time),"","height=500,width=700,top=100,left=100,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no, status=no");
            query();
...
function toSubmit(toValue){
     ...
     if(toValue == '首页'){
         StationGroup.action="<%=request.getContextPath()%>/stationGroup.action?functionName=queryGroupSecond&currentPage="+1;
         StationGroup.submit();
     }
