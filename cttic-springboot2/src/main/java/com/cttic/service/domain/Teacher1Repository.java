package com.cttic.service.domain;
...
@CacheConfig(cacheNames = "teacher1s")
public interface Teacher1Repository extends JpaRepository<Teacher1, Integer>, JpaSpecificationExecutor<Teacher1> {
	...
	/*
	 * 在spring boot中通过@EnableCaching注解自动化配置合适的缓存管理器。 Generic、JCache、EhCache。。。reids
	 * spring.chache.type强制指定缓存管理器。 可以直接在resources下配置ehcache.xml，spring
	 * boot会自己侦测到。但是为了后期的演示，我们将其放到config文件夹中，并且改名为ehcache-config.xml,然后配置 指定这个xml
	 * spring.cache.ehchache.config=classpath:cofnig/ehcache-config.xml
	 */
