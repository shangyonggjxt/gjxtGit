...
<html>
  ...
  <div class="easyui-panel" title="账号修改" style="width:600px;padding:30px 60px">
  
  <form name="operatorEdit" id="operatorEdit" method="post" action="">
  <s:hidden name="operator.operator_id" id="operator.operator_id" value="%{operator.operator_id}"></s:hidden>
  <s:hidden name="operator.operator_code" id="operator.operator_code" value="%{operator.operator_code}"></s:hidden>
    <table width="100%" cellspacing="1" bgcolor="#CCCCCC">
      ...
      <tr>
        ...
        <s:textfield name="operator.operator_pwd"
								value="%{operator.operator_pwd}" id="operator.operator_pwd" theme="simple"></s:textfield>        </td>
      </tr>
      <tr>
      ...
      <tr>
        <td colspan="4" align="center" bgcolor="#FFFFFF"><input type="button" name="Submit" value="提交" onClick="savaData()" class="easyui-linkbutton" style="width:50%;height:32px"></td>
      </tr>
    </table>
  </form>
...
function savaData(){
 ...
 operatorEdit.action=encodeURI("<%=request.getContextPath()%>/operator.action?functionName=update&time="+time);
 operatorEdit.submit();
